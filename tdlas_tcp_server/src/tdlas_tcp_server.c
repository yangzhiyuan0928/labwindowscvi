		...
		case EVENT_COMMIT:
			if(Server_registed == 1)
			{
				if(ConfirmPopup("Message Confirm","确认注销?"))
				{
					...
					SetCtrlVal(panelHandle,PANEL_CH1_LED,0);
					SetCtrlVal(panelHandle,PANEL_CH2_LED,0);
				}
			}
			else
		...
		case TCP_DATAREADY:  //Server端口有数据:上位机回发的命令
		    ...
		    switch(receiveCmd)
		    {
				...
				case 0xA5A51C1A: //SET_Notice: 制冷器温度设置、激光器电流设置超限提醒	
					...
					//显示到面板上 
					SetCtrlVal(TabPanel_1,TABPANEL_1_TEC_TEMP_MAX,paramLimit.tecTmax);  //需要将double型数据转换为String?
		    	...
		    	case 0xA5A54831://TEC_Monitor_Dat:
		    		printf("receiveCmd = %x\n",receiveCmd);
		    		monitorParam monParam; //局部变量
		    		ServerTCPRead(g_hconversation,&monParam,sizeof(monParam),1000); //接收Client发过来的监测参数
					//显示数据到面板
					SetCtrlVal(TabPanel_1,TABPANEL_1_TEC_REAL_V,monParam.tecRealV);	   //需要将double型数据转换为String?  
